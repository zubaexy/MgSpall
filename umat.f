...
c--------------------------------------------------------------------
      ...
      subroutine calc_spbas(sig, mbas, sbas, pbas)

      implicit none
      
c     input
      double precision sig(3,3), mbas(3)
            
c     output
      double precision sbas(3), pbas(3,3)
      
c     util
      double precision den, num
      double precision a(3)
      
c     a(i) = sig(i,j)*m(j) = sig(i,j)*m(i) = old num1(i)
      a(1) = sig(1,1)*mbas(1)+sig(1,2)*mbas(2)+sig(1,3)*mbas(3)
      a(2) = sig(2,1)*mbas(1)+sig(2,2)*mbas(2)+sig(3,3)*mbas(3)
      a(3) = sig(3,1)*mbas(1)+sig(3,2)*mbas(2)+sig(3,3)*mbas(3)
      
c     den = mbas(i)*sig(i,j)*sig(j,k)*mbas(k) = a(i)*a(i)
      den = a(1)**2+a(2)**2+a(3)**2
      
c     num = mbas(i)*sig(i,j)*mbas(j) = mbas(i)*a(i)
      num = mbas(1)*a(1)+mbas(2)*a(2)+mbas(3)*a(3)   

      if ((den-num**2).le.1d-10) then
...
c       sbas(i) = (num1(i) - mbas(i)*num)/ dsqrt(den-num**2)
c       set den to real denominator expression
        den = dsqrt(den-num**2)
        sbas(1) = (a(1)-num*mbas(1))/den
        sbas(2) = (a(2)-num*mbas(2))/den
        sbas(3) = (a(3)-num*mbas(3))/den
      end if
      
      pbas(1,1) = 0.5d+0*(sbas(1)*mbas(1)+sbas(1)*mbas(1))
      pbas(1,2) = 0.5d+0*(sbas(1)*mbas(2)+sbas(2)*mbas(1))
      pbas(1,3) = 0.5d+0*(sbas(1)*mbas(3)+sbas(3)*mbas(1))
      pbas(2,1) = pbas(1,2)
      pbas(2,2) = 0.5d+0*(sbas(2)*mbas(2)+sbas(2)*mbas(2))
      pbas(2,3) = 0.5d+0*(sbas(2)*mbas(3)+sbas(3)*mbas(2))
      pbas(3,1) = pbas(1,3)
      pbas(3,2) = pbas(2,3)
      pbas(3,3) = 0.5d+0*(sbas(3)*mbas(3)+sbas(3)*mbas(3))      
